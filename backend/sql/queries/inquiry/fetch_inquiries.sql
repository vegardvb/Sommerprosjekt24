...
FROM
    "Inquiry" inquiry
    INNER JOIN "InquiryMeasurement" inquiry_measurement ON inquiry.id = inquiry_measurement.inquiry_id -- Include the number of measurements related to the inquiry as a sub-query
    INNER JOIN (
        /*cable_measurements*/
    ) AS cable_measurements ON cable_measurements.inquiry_id = inquiry.id
