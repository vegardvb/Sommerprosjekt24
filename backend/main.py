...
def get_images_by_inquiry(inquiry_id: int, connection=Depends(get_db_public)):
    ...
    try:
        ...
        logger.info(
            "API call to fetch images for inquiry %s", inquiry_id
        )  # Log API call
        return result
...
def update_coordinates(
    ...
    try:
        ...
        new_geom = func.ST_SetSRID(
            func.ST_MakePoint(coordinate_update.lon, coordinate_update.lat), 4326
        )

        # Extract x and y from new_geom
        x_expr = func.ST_X(
            func.ST_Transform(new_geom, 32633)
        ...
        stmt = (
            ...
            .values(
                hoyde=coordinate_update.hoyde, metadata=updated_metadata, geom=new_geom
            )
        )
        ...
        return {
            "message": "Height, coordinates, and metadata updated successfully",
            "id": edited_point_id,
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail="An error occurred while updating the metadata"
        ) from e
